...
import { getStatusCode, getUserAge, validateEmail, getResponse, mockApiCall } from './api.js';
...
describe('New Matcher Features', () => {
  ...
  describe('Please() - Forced values with warning', () => {
    test('forces the actual value being tested', async () => {
      const result = await mockApiCall(); // Returns random number
...
describe('Mock Functions', () => {
  ...
  test('uses implementation once', () => {
    const mockFn = mock();
    mockFn.mockImplementationOnce((x: number) => x * 2);
